...
Click "Run All Tests" to see which services need configuration, then add your service details using the [example values file](helm/airia-infrastructure-validation/examples/basic-values.yaml).
...
Download the [example values file](helm/airia-infrastructure-validation/examples/basic-values.yaml), fill in your service details, and deploy:
...
helm upgrade airia-infrastructure-validation \
  oci://ghcr.io/davidpacold/airia-infrastructure-validation/charts/airia-infrastructure-validation \
...
docker run -d -p 8080:8080 \
  ...
  ghcr.io/davidpacold/airia-infrastructure-validation:latest
...
```bash
# Check pod logs
kubectl logs -n airia -l app.kubernetes.io/name=airia-infrastructure-validation

# Verify services are accessible from the pod
kubectl exec -n airia deployment/airia-infrastructure-validation -- nslookup your-server.postgres.database.azure.com

# Verify configuration was applied
helm get values airia-infrastructure-validation -n airia

# Restart pod to pick up new config
kubectl rollout restart -n airia deployment/airia-infrastructure-validation
```
...
- **[Example Values File](helm/airia-infrastructure-validation/examples/basic-values.yaml)** - Customer-ready config template
...
```bash
helm uninstall airia-infrastructure-validation -n airia
```
